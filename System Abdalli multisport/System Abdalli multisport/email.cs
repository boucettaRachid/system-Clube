...
namespace System_Abdalli_multisport
{
    class email
    {
        ...
        public void mailAdmin(string messsage)
        {
            try
            {
                ...
                using (MailMessage mail = new MailMessage())
                {
                    ...
                    mail.To.Add("rachid.bouctta1997@gmail.com"/*EmailSendr*/);
        ...
        public void mailPlayer(string mailito)
        {
            try
            {
                infomail();
                using (MailMessage mail = new MailMessage())
                {
                    ...
                    using (SmtpClient smtp = new SmtpClient(SMTP, Convert.ToInt32(Port)))
                    {
                        smtp.Credentials = new NetworkCredential(EmailAdmin, password);
                        smtp.EnableSsl = true;
                        smtp.Send(mail);
                        MessageBox.Show("Email Send Successfuly");
                    }
